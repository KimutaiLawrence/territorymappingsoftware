...
import { usePotentialLocations, useDeleteLocation, useCreateLocation, useUpdateLocation } from '@/hooks/use-api'
...
export default function PotentialLocationsPage() {
  ...
  const { data: locationsData, isLoading: locationsLoading } = usePotentialLocations(pagination)
  ...
  const handleConfirmDelete = () => {
    if (selectedLocation) {
      deleteLocation.mutate({ id: selectedLocation.id, type: 'potential' })
  ...
  const handleFormSubmit = (data: any) => {
    ...
    if (selectedLocation) {
      updateLocation.mutate({ id: selectedLocation.id, data: formattedData, type: 'potential' })
    } else {
      ...
      <DashboardLayout>
        <div className="h-full flex flex-col p-4 space-y-4">
          <div className="flex justify-between items-center shrink-0">
            <h1 className="text-2xl font-bold">Potential Locations</h1>
            {can('create', 'location') && (
